...
excerpt: "Visual Odometry pipeline tested with the KITTI dataset"
...
- Using these good matches, we filter the keypoints from each image and reconstruct points in 3-dimensional space utilizing the depth map, and filter the points in the next image.  
- Using the set of 3-dimensional points, and the filtered points in the next image, the camera's new pose is estimated using Perspective-n-Point RANSAC. 
